...
//        if (routeEvent.value?.eventType?.isRouteEnd() == true) routeEvent.postValue(null)
    ...
    fun startRoute() {
        if (sharedPreferences.getBoolean(SIMULATE_ROUTE, false)) {
            route.value?.let { replayStart(it) }
        }
    ...
    private val apiListener = object: ProximiioListener() {
        ...
        override fun position(location: Location) {
            // Do not update location if replay is in progress
            if (routeReplay != null) return
    ...
    /* ------------------------------------------------------------------------------------------ */
    /* Route replay - example of advanced  */

    private var routeReplayStartLevel: Int = 0
    private var routeReplay: RouteReplay? = null
    private var routeReplayDelay = 700L // ms
    private var handler = Handler()

    /**
     * Start replay on supplied route.
     */
    private fun replayStart(route: Route) {
        routeReplayStartLevel = userLevel.value ?: 0
        routeReplay = RouteReplay(route)
        replayScheduleNext()
        proximiioAPI.pdrOnly(true)
    }

    /**
     * Schedule next step of replay.
     */
    private fun replayScheduleNext(delay: Long = routeReplayDelay) {
        handler.removeCallbacks(replayRunnable)
        handler.postDelayed(replayRunnable, delay)
    }

    /**
     * Cancel current replay. Reset user position to original position on map.
     */
    private fun replayCancel() {
        ...
        handler.postDelayed({
            routeReplay = null
            proximiioAPI.pdrOnly(false)
            userLevel.postValue(routeReplayStartLevel)
            displayLevel.postValue(routeReplayStartLevel)
        }, routeReplayDelay * 2L)
    }

    /**
     * Runnable for steps of replay. Takes new point generated by replay and updates data accordingly.
     */
    private val replayRunnable: Runnable = Runnable {
        routeReplay?.let {
            val nextStep = it.getNext()
            nextStep?.let {
                if (it.level == userLevel.value) {
                    userLocation.postValue(it.point.toLocation())
                    replayScheduleNext(routeReplayDelay)
                } else {
                    val levelDifference = it.level - userLevel.value!!
                    val levelChangeDirection = if ((it.level - userLevel.value!!) < 0) -1 else 1
                    for (i in 1..abs(levelDifference)) {
                        val newLevel = userLevel.value!! + levelChangeDirection
                        handler.postDelayed({
                            val newLevel = userLevel.value!! + levelChangeDirection
                            if (userLevel.value == displayLevel.value) { displayLevel.postValue(newLevel) }
                            userLevel.postValue(newLevel)
                        }, 3000 * i.toLong())
                    }
                    handler.postDelayed({
                        userLocation.postValue(it.point.toLocation())
                    }, 3000 * abs(levelDifference.toLong()))
                    replayScheduleNext(3000 * (abs(levelDifference.toLong())+ 1L))
                }
            }
        }
    }
